---
title: AB实验（1）AB实验从0到1
date: 2024-05-18 18:33:48
permalink: /AB-Test/intro-1/
categories:
  - 数据科学
tags:
  - AB实验
updated: 2024-12-14 13:30:00
summary_en: A practical introduction to A/B testing, experiment design, guardrails, and how product teams should interpret evidence.
summary_zh: 从实验设计、指标到结果解读，介绍 A/B 实验从 0 到 1 的基本方法。
topic_cluster: experimentation
public_focus: Public-safe introduction to A/B testing workflow, metrics, and launch reasoning.
confidentiality_safe: true
site_locale: zh-CN
last_updated: 2026-05-31
---
