...
TBD — describes the flat work manifest (`source/_data/works.yml`) as the single source of truth for publishable artifacts, the `work_key` front-matter binding, the manifest resolution helper, the related-works rendering contract, and the build-time integrity validator.
